...
/**
 * The file format identificator action using Apache Tika library
 * @author wisanup
 * @see <a href="http://tika.apache.org/">Apache Tika website</a>
 *
 */
@XStreamAlias("tikaIdentification")
public class TikaFileIdentificationAction  extends AbstractAction{
	
	private WorkflowCache cache;
	private static final Log log = LogFactory.getLog(TikaFileIdentificationAction.class);
	 ...
	 */
	public TikaFileIdentificationAction(){
	...
	public void processAction() {
		...
		try {
			for (Entry ent: (List<Entry>)cache.getAll(Entry.class)){
				createFileIdentificationFile(StringHelper.joinDirectoryString(path , ent.getString(Entry.NAME)), StringHelper.joinDirectoryString(path , ent.getString(Entry.NAME))  + ".format.tika.xml");
			...
			CoreThreadFactory.getCallBackExecutorService().execute(new Runnable() {
				@Override
				public void run() {
					listener.onActionError(TikaFileIdentificationAction.this, e.getMessage());
				}
			});
			return;
		}
		...
		CoreThreadFactory.getCallBackExecutorService().execute(new Runnable() {
			...
			public void run() {
				...
				listener.onActionFinished(TikaFileIdentificationAction.this);
			}
		});
	}
	 ...
	 * The output file name will be automatically generated by {originalname}.format.tika.xml 
	...
	public void createFileIdentificationFiles(List<String> files) throws IOException{
		for (String file: files){
			createFileIdentificationFile(file, file + ".format.tika.xml");
	...
	public void createFileIdentificationFile(String sourceFile, String outputFile) throws IOException{
		...
		ow.writeMetadata(valueMap, "tika", "", outputFile);
	}
	...
	public String getFileIdentification(String filePath) throws IOException{
		return tika.getFileType(filePath);
	}
